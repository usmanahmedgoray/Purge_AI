...
// Route:01 => "POST" Method => API endpoint for Correct the English Grammar => URL : "http:localhost:500/correctGram"

router.post("/correctGram", parser.json(),async (req, res) => {
    //  Wrap the whole code in try catch to handle the expected error
    try {

        // Destructure the paragrapgh from body
        const para = await req.body.para;
...
// Route:02 => "GET" Method => API endpoint for Correct the English Grammar => URL : "http:localhost:500/correctGram"

router.get('/correctGram', (req, res) => {
